<template>
  <div class="item-list">
    <div class="head">
      <span><i></i>{{title}}</span>
      <router-link tag="small" to="/rank">{{more}}<i class="icon-back"></i></router-link>
    </div>
    <div class="main">
      <div class="section" v-for="(item,index) in list" @click="selectItem(item, index)" v-if="index < listNum">
        <div class="se-img">
          <div class="img">
            ...
            <div class="font"><i class="icon-play"> {{item.view_count}}</i><i class="icon-playlist m-l-30"> {{item.comment_count}}</i></div>
          </div>
          <p>{{item.soundstr}}</p>
        </div>
      </div>
